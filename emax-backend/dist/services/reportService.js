...
const getSalesReport = async (dateFrom, dateTo) => {
    ...
    const [totalOrders, pendingOrders, processingOrders, shippedOrders, deliveredOrders, cancelledOrders, revenueAgg] = await Promise.all([
        prisma_1.prisma.order.count({ where: whereDate }),
        prisma_1.prisma.order.count({ where: { ...whereDate, status: "PENDING" } }),
        prisma_1.prisma.order.count({ where: { ...whereDate, status: "PROCESSING" } }),
        prisma_1.prisma.order.count({ where: { ...whereDate, status: "SHIPPED" } }),
        prisma_1.prisma.order.count({ where: { ...whereDate, status: "DELIVERED" } }),
        prisma_1.prisma.order.count({ where: { ...whereDate, status: "CANCELLED" } }),
...
const getFinanceReport = async (dateFrom, dateTo) => {
    const from = dateFrom ? new Date(dateFrom) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
    const to = dateTo ? new Date(dateTo) : new Date();
    const whereDate = { createdAt: { gte: from, lte: to } };
    const [paidOrders, refundedOrders, revenueAgg] = await Promise.all([
        prisma_1.prisma.order.count({ where: { ...whereDate, paymentStatus: "PAID" } }),
        prisma_1.prisma.order.count({ where: { ...whereDate, paymentStatus: "REFUNDED" } }),
        prisma_1.prisma.order.aggregate({
            _sum: { totalAmount: true },
...
const getInventoryReport = async () => {
    const [totalProducts, totalStock, outOfStock, lowStock] = await Promise.all([
        ...
        prisma_1.prisma.product.aggregate({ _sum: { stock: true } }),
        prisma_1.prisma.product.count({ where: { stock: 0 } }),
        prisma_1.prisma.product.count({ where: { stock: { gt: 0, lte: 10 } } }),
    ]);
...
const getCustomerReport = async (dateFrom, dateTo) => {
    const from = dateFrom ? new Date(dateFrom) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
    const to = dateTo ? new Date(dateTo) : new Date();
    const whereDate = { createdAt: { gte: from, lte: to } };
    const [cancelledOrders, refundedOrders, totalReviews, reviews] = await Promise.all([
        prisma_1.prisma.order.count({ where: { ...whereDate, status: "CANCELLED" } }),
        prisma_1.prisma.order.count({ where: { ...whereDate, paymentStatus: "REFUNDED" } }),
        prisma_1.prisma.productReview.count({ where: whereDate }),
...
const getPerformanceReport = async (dateFrom, dateTo) => {
    const from = dateFrom ? new Date(dateFrom) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
    const to = dateTo ? new Date(dateTo) : new Date();
    const whereDate = { createdAt: { gte: from, lte: to } };
    const [totalOrders, deliveredOrders, cancelledOrders, totalReviews, totalRatingAgg] = await Promise.all([
        ...
        prisma_1.prisma.order.count({ where: { ...whereDate, status: "CANCELLED" } }),
        prisma_1.prisma.productReview.count({ where: whereDate }),
        prisma_1.prisma.productReview.aggregate({
            _avg: { rating: true },
            where: whereDate,
        }),
    ]);
